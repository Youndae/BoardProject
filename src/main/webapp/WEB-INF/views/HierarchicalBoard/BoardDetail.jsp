	...
	<c:forEach var="clist" items="${comment}">
		<div id="comment">
			...
			<c:set var="cId" value="${clist.userId}"/>
				...
				<button type="button" id="DeleteComment" value="${clist.commentNo}" onclick="DelComment(this)">삭제</button></p>				
				</c:if>
				</c:when>
				<c:otherwise>
				<p>
	...
	<div>
				<ul>
					<c:if test="${pageMaker.prev}">
						<li><a
							href="/board/BoardDetail${pageMaker.makeQuery(pageMaker.startPage - 1)}">이전</a></li>
					</c:if>

					<c:forEach begin="${pageMaker.startPage}"
						end="${pageMaker.endPage}" var="idx">
						<li><a href="/board/BoardDetail?boardNo=${boardDetail.boardNo}&${pageMaker.makeQuery(idx)}">${idx}</a></li>
					</c:forEach>

					<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
						<li><a
							href="/board/BoardDetail${pageMaker.makeQuery(pageMaker.endPage + 1)}">다음</a></li>
					</c:if>
				</ul>
			</div>
</body>
